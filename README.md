...
Build the recorded SMHasher3 implementations and measure the 43 timed rows in
...
For the preselected ten-hash verification panel, including scratch registrations:
...
komihash, ChainHash v1/v3/128, HalftimeHash24 shipped, MuseAir v2, foldhash-fast,
and GoMapHash. Entries execute in manifest order, in complete passes, rather
...
```sh
python3 scripts/benchmark.py --host Xeon8375C --names XXH3-64 chain-v3 --out out/custom --output custom-speeds.json
```
...
the article's names. `chainhash-x86` is the cross-host record key for the v2 row;
its M2 `registered_name` is `chainhash-adjacent-1k`.
...
2. ChainHash v1, foldhash, OpenSSL proxies, and native ARM UMASH/CLHASH adapters.
...
6. ChainHash v2, v3 and the original strided 128-bit construction.
...
ARM additionally applies `patches/series.arm64`: a small build-system fix passing
`CMAKE_SYSTEM_PROCESSOR` into the separate Hashrefs generation process. Without
it, that process selects the x86 family list while the ARM compiler builds the
ARM family list, leaving an unresolved registration reference. The Xeon source
and object configuration do not need this additional patch.
...
five observations. The retiming gate was relaxed to 4.5. For example, the old
ChainHash-1k run reported 48.46 B/cycle versus 17.70 in its companion run, with
small-key cost moving inversely (28.76 versus 74.43). That correlated change is
consistent with a calibration/frequency-scheduling artefact, not a corresponding
change in hashing throughput. The calibration multiplier was not printed, so
its exact cause cannot be reconstructed from the logs.
...
|---|---|
| ChainHash v1 | SplitMix64 fills the strided PH and finalizer key outside timing; 256-byte blocks (the build also registers 1 KiB). |
| ChainHash v2 | `key_from_seed` expansion outside timing; adjacent pairs, 1 KiB blocks. Xeon runtime SIMD and M2 LD2/PMULL implement the same selected family. |
| ChainHash v3 | SplitMix64 produces the 64-byte input to the v3 key expander before timing; 256-byte Horner blocks. This is the archived v3 implementation, not a subsequently tuned version. |
| ChainHash-128 | `key_from_splitmix64`, outside timing; original 512-byte strided construction and three-product Karatsuba. The schoolbook alternative is built but not substituted into the chart row. |
...
fails outside ±5%. See [verification/REPORT.md](verification/REPORT.md) for the
actual clean-build Xeon panel and raw evidence. All ten bulk results met ±5%
...
audit compares pre-link `.text` sections of SpeedTest and four original control
